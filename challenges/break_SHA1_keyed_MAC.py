...
def construct_new_MAC(known_message, MAC, message_suffix, key_length = 16):
  ...
  return new_message, hashes.SHA1(message_suffix, original_byte_len = key_length + len(new_message), init_state = init_regs)

def verify_message(key, message, MAC):
  return hashes.SHA1(key + message) == MAC
...
if __name__ == '__main__':
  ...
  MAC = hashes.SHA1(key + known_message)
